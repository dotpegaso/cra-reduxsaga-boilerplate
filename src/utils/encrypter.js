...
class Encrypter {
  constructor(key, cipher = 'AES-128-CBC') {
    ...
    if (Encrypter.supported(key, cipher)) {
      ...
      throw `The only supported ciphers are [${Object.keys(
        Encrypter.getCipher()
      ).join(', ')}] with the correct key lengths.`
    }
  }
  ...
  encrypt(value, serialize = true) {
    ...
    try {
      ...
      throw 'Could not encrypt the data.'
    }
    ...
    return Buffer.from(JSON.stringify({ iv, value, mac }), 'utf8').toString(
      'base64'
    )
  }
  ...
  decrypt(payload, unserialize = true) {
    ...
    try {
      ...
      throw 'Could not decrypt the data.'
    }

    try {
      return unserialize
        ? Serialize.unserialize(decryptedPayload)
        : decryptedPayload
    } catch (e) {
      throw 'Could not unserialize the data.'
    }
  }
  ...
  validMac(payload) {
    ...
    return timingSafeEqual(hmac.digest(), calculated)
  }
...
export default new Encrypter(
  Buffer.from(CONFIG.ENCRYPTER_KEY.substr(7), 'base64')
)
